...
def feat_signal_gazole(cheval, hippo_nom):
    """
    B4 — Signal gazole : entraîneur qui se déplace de loin pour cette course.
    Les entraîneurs du sud à Paris ou province eloignée ont un fort taux de réussite.
    Proxy : si l adresse/région de l entraîneur est disponible dans l API.
    Sinon : on détecte via le nom de l écurie si elle contient une ville éloignée.
    """
    if not isinstance(cheval, dict): return 0
    hippo = str(hippo_nom or "").upper()

    # Hippodromes "parisiens" ou du nord
    hippo_paris = any(x in hippo for x in [
        "VINCENNES", "ENGHIEN", "AUTEUIL", "LONGCHAMP",
        "SAINT-CLOUD", "MAISONS", "COMPIEGNE", "CHANTILLY"
    ])

    entraineur = cheval.get("entraineur") or {}
    if not isinstance(entraineur, dict): return 0

    region = str(
        entraineur.get("region") or entraineur.get("localite") or
        entraineur.get("commune") or entraineur.get("ville") or ""
    ).upper()

    # Régions du sud / ouest / loin de Paris
    regions_loin = [
        "MARSEILLE", "TOULOUSE", "BORDEAUX", "NICE", "MONTPELLIER",
        "NANTES", "RENNES", "BREST", "STRASBOURG", "LYON",
        "GRENOBLE", "TOULON", "NIMES", "PERPIGNAN", "PAU",
        "TARBES", "BAYONNE", "CAEN", "LE MANS", "ANGERS",
    ]

    if hippo_paris and region and any(r in region for r in regions_loin):
        return 5  # Entraîneur du sud qui monte à Paris = signal fort

    # Proxy secondaire : si l écurie contient une ville lointaine
    ecurie = str(cheval.get("ecurie") or cheval.get("nomEcurie") or "").upper()
    if hippo_paris and ecurie and any(r in ecurie for r in regions_loin):
        return 3
...
def compute_logit(cheval, course, tous_partants, disc):
    ...
    f_gazole       = feat_signal_gazole(cheval, hippo_code)
...
with st.sidebar:
    ...
    st.caption("**🆕 Bloc B :** Jockey/hippo · Trainer/discipline · Combo enrichi · Gazole · Tendance 3 mois · Repos optimal · Course prépa")
