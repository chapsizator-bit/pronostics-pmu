...
def analyse_course(partants, course):
    ...
    for d in donnees:
        ...
        resultats.append({
            ...
            "prob":          prob,
            "prob_mkt":      prob_mkt,
            "value":         0,   # plus utilisé pour trier
            "confiance":     confiance,
            "cote":          cote,
            "logit":         round(logit, 1),
            "details":       d["details"],
...
with st.sidebar:
    ...
    min_conf   = st.slider("Confiance minimum", 0, 99, MIN_CONF,
                           help="Score Benter objectif — stable toute la journée")
...
if st.button("🔍 Analyser les courses du jour", use_container_width=True, type="primary"):
    ...
    gardes = [
        ...
        if c["confiance"] >= min_conf
        and c.get("cote")
    ]
    gardes.sort(key=lambda x: x["logit"], reverse=True)
    ...
    if not gardes:
        ...
        if top3:
            ...
            for c in top3:
                st.caption(f"• N°{c['num']} {c['nom']} ({c.get('course','?')}) — Score {c['logit']} · Confiance {c['confiance']}% · Cote {c.get('cote','?')}")
    else:
        medals = ["🥇", "🥈", "🥉"] + ["🔹"] * 10
        ...
        for i, c in enumerate(gardes):
            ...
            col3.metric("Cote (info)",  f"{c['cote']}×" if c.get('cote') else "—")
            col4.metric("Prob. modèle", f"{c['prob']}%")
            ...
            if partenaires:
                nums_part = [f"N°{p['num']}" for p in partenaires]
                cotes_part = [f"{p['cote']}" for p in partenaires]
                base_str  = f"N°{base_num} {c['nom'][:15]}"

                # Couplé gagnant
                couples = [f"**{base_str}** × **N°{p['num']} {p['nom'][:12]}** (cote {p['cote']})" for p in partenaires[:3]]

                st.markdown(f"""
                <div style="background:#0d1f0d;border:1px solid #1a3a1a;border-radius:10px;padding:14px;margin-bottom:8px">
                  <div style="font-size:11px;color:#4ade80;font-weight:800;text-transform:uppercase;letter-spacing:1px;margin-bottom:10px">
                    🎯 Paris conseillés
                  </div>
                  <div style="margin-bottom:8px">
                    <span style="color:#fbbf24;font-weight:700;font-size:12px">🏆 BASE :</span>
                    <span style="color:#f1f5f9;font-size:13px;font-weight:800"> N°{base_num} {c['nom']} (cote {c['cote']})</span>
                  </div>
                  <div style="margin-bottom:6px">
                    <span style="color:#60a5fa;font-weight:700;font-size:11px">🔗 Couplé Gagnant :</span>
                    <span style="color:#94a3b8;font-size:11px"> Base × {" / Base × ".join([f"N°{p['num']} {p['nom'][:10]} ({p['cote']})" for p in partenaires[:3]])}</span>
                  </div>
                  <div style="margin-bottom:6px">
                    <span style="color:#a78bfa;font-weight:700;font-size:11px">📋 Tiercé désordre :</span>
                    <span style="color:#94a3b8;font-size:11px"> N°{base_num} - {" - ".join([f"N°{p['num']}" for p in partenaires[:2]])}</span>
                  </div>
                  <div>
                    <span style="color:#34d399;font-weight:700;font-size:11px">🎰 Multi {len(partenaires)+1} chevaux :</span>
                    <span style="color:#94a3b8;font-size:11px"> N°{base_num} + {" + ".join([f"N°{p['num']}" for p in partenaires])}</span>
                    <span style="color:#475569;font-size:10px"> ({len(partenaires)} combinaisons couplé)</span>
                  </div>
                </div>
                """, unsafe_allow_html=True)
            else:
                st.caption("💡 Pas assez de partenaires (cote 4-22) dans cette course pour un couplé.")

            st.markdown("---")

    st.caption(f"Tri : Score Benter objectif · Confiance ≥ {min_conf}% · Cote = info seulement")
