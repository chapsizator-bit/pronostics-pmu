...
def get_letrot_data(nom_cheval):
    """LeTROT désactivé — bloque les requêtes depuis Streamlit Cloud."""
    return None
...
def feat_taux_victoires(ch):
    if not isinstance(ch, dict): return 0
...
def feat_vict_hippo(ch, hippo):
    if not isinstance(ch, dict) or not hippo: return 0
...
def feat_vict_dist(ch, course):
    if not isinstance(course, dict) or not isinstance(ch, dict): return 0
...
def feat_oeilleres(ch, disc):
    if not isinstance(ch, dict) or disc=="TROT": return 0
...
def feat_corde_plat(ch, course, disc):
    if not isinstance(ch, dict) or not isinstance(course, dict) or disc!="PLAT": return 0
...
def feat_marche(ch):
    if not isinstance(ch, dict): return 0, None
...
def feat_outsider(ch, tous, disc):
    if not isinstance(ch, dict): return 0, False
    """
...
def compute(ch, course, tous, disc, meteo=None, letrot_data=None):
    if not isinstance(ch, dict): return 0, None, {}, False
    if not isinstance(course, dict): course = {}
    if not isinstance(tous, list): tous = []
    music=parse_musique(ch.get("musique",""))
    deb=len(music)==0
...
def analyser_course(partants, course, reunion, meteo=None):
    ...
    for ch in partants:
        ...
        # Enrichissement LeTROT désactivé (blocage réseau Streamlit Cloud)
        letrot_data=None
        try:
