...
SEL_RAW      = "https://raw.githubusercontent.com/chapsizator-bit/pronostics-pmu/main/selections.json"
...
def save_selections_github(data):
    ...
    sha = None
    try:
        req = urllib.request.Request(SEL_API, headers=headers)
        with urllib.request.urlopen(req, timeout=8) as r:
    ...
    try:
        req2 = urllib.request.Request(
            SEL_API, data=json.dumps(payload).encode(),
...
def feat_performance_relative(cheval, tous_partants):
    """Score absolu basé sur taux de victoire — sans cote ni champ."""
    total = safe_int(cheval.get("nombreCourses", 0))
    vict  = safe_int(cheval.get("nombreVictoires", 0))
    if total < 5: return 0
    taux = vict / total
    if taux > 0.30:   return 5
    elif taux > 0.20: return 3
    elif taux > 0.15: return 1
    return 0
...
col_jour, col_btn, col_force = st.columns([1.5, 3, 1])
analyse_demain = col_jour.radio(
    "Analyser",
    ["📅 Aujourd'hui", "📆 Demain"],
    label_visibility="collapsed",
    help="Le soir, analysez les courses de demain"
) == "📆 Demain"
date_cible     = datetime.now() + dt_mod.timedelta(days=1) if analyse_demain else datetime.now()
date_str_cible = date_cible.strftime("%d%m%Y")
date_label     = date_cible.strftime("%A %d %B")

lancer = col_btn.button(f"🔍 Analyser — {date_label}", use_container_width=True, type="primary")
force  = col_force.button("↺ Forcer", help="Vide le cache et recalcule")

# Vider le cache si on force
if force:
    for k in list(st.session_state.keys()):
...
if cached and cached.get("date") == date_str_cible and not force:
    ...
    st.markdown(
        f'<div class="frozen">🔒 Sélections de {label_cache} figées à {cached["heure"]} · {github_ok} · ↺ Forcer pour recalculer.</div>',
        unsafe_allow_html=True
    )
    st.session_state["gardes_du_jour"] = cached["gardes"]
    ...
    if not gardes:
        ...
        for i, c in enumerate(gardes):
            ...
            pills = "".join([pill(k, d.get(v)) for k, v in [
                ("Forme","forme"),("Rég.","reg"),("Prog.","prog"),
                ("Classe","classe"),("RK","rk"),("Distance","dist_affinee"),
                ("Rég.cond.","reg_cond"),("Fraîcheur","repos_opt"),
                ("Déferré","deferre"),("Jockey","jock_hippo"),
                ("Driver★","driver_qual"),("Train.★","trainer_qual"),
                ("Spéc.★","gazole"),("Poids","poids"),("Œill.","oeil"),
                ("Sexe★","sexe_disc"),("Hippo","hippo_fiab"),
                ("Podiums","serie_pod"),("Cl.↓","baisse_cl"),
                ("Mission","mission"),("Saison","vict_mois"),
            ]])
            hippo_str = f" · {c['hippo']}" if c.get("hippo") else ""
            st.markdown(f"""
            <div class="card">
              <div class="horse-name">{medals[i]} N°{c['num']} {c['nom']} {badge_hippo}</div>
              <div class="sub">{c.get('course','?')} · {c.get('discipline','?')} {hippo_str}</div>
    ...
    with st.spinner(f"Chargement — {date_label}…"):
        try:
            data     = pmu_get(f"/{date_str_cible}/reunions")
    ...
    for i, (r, c) in enumerate(courses):
        ...
        # Filtre discipline (sidebar)
        if disc_c_check not in disc_filtre:
            continue
        ...
        if not num_r or not num_c:
            continue
        try:
            pdata    = pmu_get(f"/{date_str_cible}/R{num_r}/C{num_c}/participants")
    ...
    if not gardes:
        ...
        top3 = sorted(candidats, key=lambda x: x["logit"], reverse=True)[:3]
        if top3:
